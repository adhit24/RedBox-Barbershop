-- Regular payroll: attendance-wide serialization, stale-attendance marker, lock backstop (round 7, P1-3)
--
-- WHY: the previous protocol (20260921033925) took part in the payroll_runs synchronization only when
-- employee_attendance.overtime_minutes changed. The payroll engine (regularPayrollService.
-- fetchEmployeeAttendanceSummaries) reads SEVEN attendance fields:
--     employee_id, attendance_date, status, late_minutes, overtime_minutes, first_check_in, last_check_out
-- (present/absent/late counts and minutes, incomplete-punch counts, coverage, candidate overtime). A
-- correction to late_minutes / status / punches with overtime unchanged therefore changed what a DRAFT item
-- SHOULD contain without the item knowing, and lock_payroll_run() could freeze the stale snapshot.
--
-- MODEL (same pattern as adjustments_dirty):
--   * trg_attendance_payroll_sync fires BEFORE INSERT / DELETE / UPDATE OF <the seven fields> and, when a
--     payroll-relevant value really changes (IS DISTINCT FROM; import re-upserts of identical rows and
--     changes of raw_punches / source / notes / batch id do nothing):
--       1. takes SHARE locks on the covering DRAFT runs (serialize_regular_payroll_mutation, ORDER BY id) so
--          it is ordered against lock_payroll_run() (FOR UPDATE) - the run row remains the sync object;
--       2. marks every DRAFT regular item of that employee whose run covers the date:
--          attendance_summary.attendance_dirty = true (same transaction as the attendance write);
--       3. LOCKED runs are NEVER mutated. A change that touches a locked payroll is recorded in
--          payroll_attendance_post_lock_anomalies (append-only) so it can be reviewed - attendance history
--          itself stays correctable.
--   * lock_payroll_run() rejects a DRAFT item with attendance_dirty (new invariant).
--   * Recalculation (service) re-reads attendance and rebuilds attendance_summary without the marker.
-- Either the writer commits first (lock RPC sees the marker after acquiring the run lock) or the lock RPC
-- commits first (writer then sees a LOCKED run and only records an anomaly). No interleaving loses a change.
--
-- Applied migrations are NOT edited. This file: one table, one helper, one trigger function, three triggers
-- (replacing the overtime-only ones) and lock_payroll_run() re-created with the new invariant only.
-- Execute rights: service_role only; no privilege widening.

BEGIN;

-- 1. Append-only anomaly log for attendance changes that touch a LOCKED payroll ---------------------
CREATE TABLE IF NOT EXISTS public.payroll_attendance_post_lock_anomalies (
...
CREATE INDEX IF NOT EXISTS idx_payroll_att_anomalies_run ON public.payroll_attendance_post_lock_anomalies (payroll_run_id);
ALTER TABLE public.payroll_attendance_post_lock_anomalies ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.payroll_attendance_post_lock_anomalies FROM PUBLIC, anon, authenticated;

-- 2. Effect of ONE payroll-relevant attendance change ------------------------------------------------
...
BEGIN
    ...
    SET attendance_summary = jsonb_set(COALESCE(i.attendance_summary, '{}'::jsonb), '{attendance_dirty}', 'true'::jsonb, true)
...
-- 3. Trigger function: only payroll-relevant changes take part -----------------------------------------
CREATE OR REPLACE FUNCTION public.trg_attendance_payroll_sync()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
DECLARE
    v_existing public.employee_attendance%ROWTYPE;
BEGIN
    IF TG_OP = 'DELETE' THEN
        PERFORM public.apply_attendance_payroll_effect(
            OLD.employee_id, OLD.attendance_date, 'DELETE',
            jsonb_build_object('status', OLD.status, 'late_minutes', OLD.late_minutes, 'overtime_minutes', OLD.overtime_minutes,
                               'first_check_in', OLD.first_check_in, 'last_check_out', OLD.last_check_out),
            NULL);
        RETURN OLD;
    END IF;

    IF TG_OP = 'INSERT' THEN
        -- INSERT .. ON CONFLICT DO UPDATE fires this trigger for the proposed row even when the conflict
        -- resolves to an update: an identical re-import must not dirty payroll.
        SELECT * INTO v_existing
        FROM public.employee_attendance a
        WHERE a.employee_id = NEW.employee_id AND a.attendance_date = NEW.attendance_date;
        IF FOUND
           AND v_existing.status IS NOT DISTINCT FROM NEW.status
           AND v_existing.late_minutes IS NOT DISTINCT FROM NEW.late_minutes
           AND v_existing.overtime_minutes IS NOT DISTINCT FROM NEW.overtime_minutes
           AND v_existing.first_check_in IS NOT DISTINCT FROM NEW.first_check_in
           AND v_existing.last_check_out IS NOT DISTINCT FROM NEW.last_check_out THEN
            RETURN NEW;
        END IF;
        PERFORM public.apply_attendance_payroll_effect(
            NEW.employee_id, NEW.attendance_date, 'INSERT', NULL,
            jsonb_build_object('status', NEW.status, 'late_minutes', NEW.late_minutes, 'overtime_minutes', NEW.overtime_minutes,
                               'first_check_in', NEW.first_check_in, 'last_check_out', NEW.last_check_out));
        RETURN NEW;
    END IF;

    -- UPDATE: relevant only when one of the seven fields the payroll engine reads really changed
    IF NEW.employee_id IS NOT DISTINCT FROM OLD.employee_id
       ...
       AND NEW.first_check_in IS NOT DISTINCT FROM OLD.first_check_in
       AND NEW.last_check_out IS NOT DISTINCT FROM OLD.last_check_out THEN
        RETURN NEW;
    END IF;

    PERFORM public.apply_attendance_payroll_effect(
        OLD.employee_id, OLD.attendance_date, 'UPDATE',
        jsonb_build_object('status', OLD.status, 'late_minutes', OLD.late_minutes, 'overtime_minutes', OLD.overtime_minutes,
                           'first_check_in', OLD.first_check_in, 'last_check_out', OLD.last_check_out),
        jsonb_build_object('status', NEW.status, 'late_minutes', NEW.late_minutes, 'overtime_minutes', NEW.overtime_minutes,
                           'first_check_in', NEW.first_check_in, 'last_check_out', NEW.last_check_out));
    IF NEW.employee_id IS DISTINCT FROM OLD.employee_id OR NEW.attendance_date IS DISTINCT FROM OLD.attendance_date THEN
        PERFORM public.apply_attendance_payroll_effect(
            NEW.employee_id, NEW.attendance_date, 'UPDATE', NULL,
            jsonb_build_object('status', NEW.status, 'late_minutes', NEW.late_minutes, 'overtime_minutes', NEW.overtime_minutes,
                               'first_check_in', NEW.first_check_in, 'last_check_out', NEW.last_check_out));
    END IF;
    RETURN NEW;
END;
$$;
REVOKE ALL ON FUNCTION public.trg_attendance_payroll_sync() FROM PUBLIC, anon, authenticated;

-- 4. Replace the overtime-only triggers ---------------------------------------------------------------
...
DROP FUNCTION IF EXISTS public.trg_attendance_overtime_serialize();

DROP TRIGGER IF EXISTS trg_attendance_payroll_sync_ins ON public.employee_attendance;
CREATE TRIGGER trg_attendance_payroll_sync_ins
BEFORE INSERT ON public.employee_attendance
FOR EACH ROW EXECUTE FUNCTION public.trg_attendance_payroll_sync();

DROP TRIGGER IF EXISTS trg_attendance_payroll_sync_upd ON public.employee_attendance;
CREATE TRIGGER trg_attendance_payroll_sync_upd
BEFORE UPDATE OF employee_id, attendance_date, status, late_minutes, overtime_minutes, first_check_in, last_check_out
ON public.employee_attendance
FOR EACH ROW EXECUTE FUNCTION public.trg_attendance_payroll_sync();

DROP TRIGGER IF EXISTS trg_attendance_payroll_sync_del ON public.employee_attendance;
CREATE TRIGGER trg_attendance_payroll_sync_del
BEFORE DELETE ON public.employee_attendance
FOR EACH ROW EXECUTE FUNCTION public.trg_attendance_payroll_sync();

-- 5. lock_payroll_run(): identical to 20260921061154 + the attendance_dirty invariant ------------------
