...
--
-- NOT APPLIED TO PRODUCTION BY THIS CHANGE — for Aira review only, per the
-- incident task's explicit instruction. Independent of, and safe to apply
-- either before or after, server/migrations/2026-08-29-wa-conversation-idle-
-- lifecycle.sql (itself also not yet applied, per its own header) — this
-- migration never references that one's new wa_conversations columns
-- (conversation_status, idle_close_due_at, etc), only adds its own.
BEGIN;
...
-- manual DDL (or 2026-08-29-wa-conversation-idle-lifecycle.sql's ALTERs) has
