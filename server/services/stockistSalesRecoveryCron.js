...
const { processMokaSale } = require('./stockistMokaSync');
const { aggregateDailyMovements, getWIBDate, getYesterdayWIBDate } = require('./stockistDailyMovements');
...
async function loadOutletContext(supabase, outlet, ownerId) {
  ...
  if (isStale && baseline?.approved_at) {
    // The approved physical count is the authoritative balance baseline.
    // Recovery must never replay older sales into today's balance.
    sinceMs = new Date(baseline.approved_at).getTime();
  } else if (Number.isFinite(lastSuccessMs)) {
    // Intentional overlap: unique sale/idempotency guards make this safe and
    // it heals transactions that Moka exposed late or a previous run missed.
...
async function stampSaleMovementTime(supabase, saleId, occurredAt) {
  ...
  const { data: items, error: itemError } = await supabase
    .from('moka_stockist_sale_items').select('id').eq('sale_id', saleId);
...
async function syncOutlet(supabase, outlet, ownerId) {
  ...
  const stats = {
    fetched: 0, processed: 0, skipped_duplicate: 0, ignored_non_stock: 0,
  ...
  try {
    for (let page = 1; page <= MAX_PAGES; page += 1) {
      ...
      for (const payment of payments) {
        ...
        } else if (['SKIPPED_DUPLICATE','SKIPPED_EXISTING','SKIPPED_CONCURRENT'].includes(result.action)) {
    ...
    await supabase.from('moka_stockist_sync_state').upsert({
      outlet_id: outlet.id, last_status: finalStatus, last_error: null,
...
async function stockistSalesRecoveryHandler(req, res) {
  ...
  try {
    ...
    if (!owner?.id) throw new Error('owner user required for Stockist movement audit');
    ...
    return res.status(failed ? 207 : 200).json({
      ok: failed === 0, page_size: PAGE_SIZE, outlets: results,
      daily_movements: movementResults.map(r => ({
        ok: r.ok, target_date: r.target_date,
        movements_processed: r.movements_processed, records_upserted: r.records_upserted,
        error: r.error,
      })),
...
module.exports = { stockistSalesRecoveryHandler, syncOutlet, parseNextSince };
