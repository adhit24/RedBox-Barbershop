-- ====================================================================================================
-- Migration: 20260922080000_regular_payroll_workforce_lock_and_manual_overtime.sql
-- Description:
--   Codex round-14 findings on PR #98 (against head 05eae78453819c6a20427bb0786e89356d3a393b):
--
--   1. PRRT_kwDOSNmW7c6klyj1 (P1): the workforce-revision capture-then-recheck added in
--      20260922060000 compared two UNLOCKED reads of payroll_workforce_version.revision. A
--      concurrent employees update could reach its AFTER trigger and commit the revision bump
--      strictly between the "before" read and the final "after" read/compare, with nothing left to
--      notice it -- the run could still lock with a stale population. The identical gap existed in
--      add_regular_payroll_run_items.
--
--      Fix: lock_payroll_run and add_regular_payroll_run_items now take `SELECT ... FOR UPDATE` on
--      the payroll_workforce_version singleton row at the START of their workforce-relevant work and
--      hold it for the REST of the function -- released only when the enclosing transaction commits
--      or rolls back (a function body executes inside the calling statement's single transaction; it
--      cannot commit internally). bump_payroll_workforce_version() (the employees trigger) already
--      issues a plain `UPDATE ... WHERE id = 1`, and in Postgres an UPDATE always takes an exclusive
--      row lock on the rows it touches, held until that transaction ends -- so the trigger already
--      "participates" in the same lock via ordinary MVCC row locking; no trigger change is needed.
--      A concurrent employees transaction that reaches this trigger while we hold FOR UPDATE blocks
--      on that row lock and therefore CANNOT COMMIT (its own COMMIT is still pending on this
--      statement) until we release -- exactly the required invariant. The revision counter itself is
--      kept as audit/version evidence (still incremented by the trigger); the row lock is the
--      correctness guarantee, so the old capture/compare/RAISE pattern is removed as dead code (it
--      can no longer fire once the lock is held for the whole window).
--
--   2. PRRT_kwDOSNmW7c6klyj8 (P2): after the round-13/14 override-precedence fix, an explicit
--      manual overtime override correctly becomes the authoritative snapshot
--      (attendance_summary.overtime_source = 'MANUAL_OVERRIDE'), but lock_payroll_run's per-item
--      overtime check still compared the snapshot minutes against the SUM of APPROVED
--      employee_overtime_approvals rows -- which is 0 (or whatever routine approvals exist) for an
--      override with no backing approval row, so every lock attempt reported a stale overtime
--      snapshot for that employee.
--
--      Fix: when attendance_summary->>'overtime_source' = 'MANUAL_OVERRIDE', the per-item loop no
--      longer compares against the approved-overtime aggregate. Instead it validates the override
--      snapshot deterministically: overtime_amount must equal ROUND(snapshot_minutes / 60.0 *
--      overtime_rate), rounded only at the final whole-Rupiah step -- this still rejects a
--      stale/tampered override (minutes edited without recalculating the amount, or vice versa)
--      without requiring a matching approval row. A normal (non-override) item is completely
--      unaffected: it still requires exact equality with the approved-overtime aggregate. Node's
--      evaluateOvertimeLockInvariants implements the identical branch (server-side change, no
--      migration needed there).
--
-- 20260922060000_regular_payroll_workforce_revision.sql (and all earlier migrations) are already
-- applied to production (khcvklzxfohwkyocenaf) and are NOT edited here. This is a new, forward-only
-- migration.
--
-- Invariants preserved:
--   - LOCKED payroll items/runs are never mutated (check_payroll_run_not_locked immutability preserved)
--   - Barber payroll branch preserved verbatim (never touches payroll_workforce_version or overtime_source)
--   - SECURITY INVOKER preserved
--   - Execute rights: service_role only (no widening)
--   - No existing applied migration is edited
--   - Existing advisory locks / attendance / adjustment CAS unchanged
-- ====================================================================================================

BEGIN;

-- 1. add_regular_payroll_run_items: hold the workforce-version row lock through the insert -------------
...
BEGIN
    ...
    -- Workforce serialization (P1, PRRT_kwDOSNmW7c6klyj1): hold the singleton row lock for the rest of
    -- this transaction. Any concurrent employees UPDATE whose AFTER trigger reaches
    -- bump_payroll_workforce_version() will block on this SAME row until we commit or roll back, so it
    -- cannot make its eligibility change visible while we are mid-reconciliation -- this is the
    -- correctness guarantee (not a capture/recheck of the revision value).
    ...
    FOR v_item IN SELECT * FROM jsonb_populate_recordset(NULL::public.payroll_regular_items, p_items) LOOP
        ...
        -- eligibility snapshot might be. While we hold the workforce-version row lock above, no
        -- concurrent employees change affecting this population can have committed since we acquired
        -- it, so this read is guaranteed consistent for the remainder of this transaction.
        IF NOT EXISTS (
...
-- 2. lock_payroll_run: hold the workforce-version row lock; manual-override overtime invariant --------
...
DECLARE
    ...
    v_expected_override_amount NUMERIC := 0;
...
BEGIN
    ...
    IF v_run.payroll_type IN ('REGULAR', 'REGULAR_PAYROLL') THEN
        -- Workforce serialization (P1, PRRT_kwDOSNmW7c6klyj1): hold the singleton row lock for the rest
        -- of this transaction, acquired BEFORE any population/guard validation. Any concurrent
        -- employees UPDATE whose AFTER trigger reaches bump_payroll_workforce_version() will block on
        -- this SAME row until we commit or roll back -- it cannot commit its eligibility change while we
        -- are mid-validation. This is the correctness guarantee; the revision counter itself remains
        -- audit/version evidence only.
        PERFORM 1 FROM public.payroll_workforce_version WHERE id = 1 FOR UPDATE;
        ...
        -- 2a-2. Population completeness backstop: every employee CURRENTLY eligible for this run's
        -- business unit/period must already have an item. Consistent for the rest of this transaction
        -- because we hold the workforce-version row lock above.
        ...
        -- 2a-3. Population two-way equality, the OTHER direction: an existing item whose employee is no
        -- longer eligible must reject the lock too, independent of whether recalculateRegularPayrollRun
        -- (and its population_changed marker) ever ran.
        ...
        -- - overtime: a MANUAL_OVERRIDE item is validated deterministically against its own rate/minutes
        --   (P2, PRRT_kwDOSNmW7c6klyj8); every other item must still equal the approved-overtime
        ...
        FOR v_snap_item IN
            SELECT i.id,
                   ...
                   COALESCE(i.overtime_rate, 0) AS overtime_rate,
                   COALESCE(i.overtime_amount, 0) AS overtime_amount,
                   (i.attendance_summary ->> 'overtime_source') AS overtime_source,
        ...
        LOOP
            ...
            IF v_snap_item.overtime_source = 'MANUAL_OVERRIDE' THEN
                -- Manual override: never compared against the DB approval aggregate (there may be no
                -- backing approval row at all -- that is the point of an override). Validated
                -- deterministically instead: overtime_amount must correspond EXACTLY to the snapshot
                -- minutes at the item's own rate, rounded only at the final whole-Rupiah step. This
                -- still catches a stale/tampered override snapshot.
                v_expected_override_amount := ROUND(v_snap_item.snap_minutes / 60.0 * v_snap_item.overtime_rate);
                IF v_expected_override_amount <> v_snap_item.overtime_amount THEN
                    RAISE EXCEPTION 'Payroll manual overtime override is inconsistent for %. Recalculate before locking.',
                        v_snap_item.employee_name_snapshot;
                END IF;
            ELSE
