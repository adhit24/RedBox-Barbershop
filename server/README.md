...
## Step 2: Setup Airtable (Optional Sync)
...
2. Buat tabel bernama **Bookings** dengan kolom berikut (Case Sensitive):
   ...
   - `Notes` (Long text)
3. Ambil **API Key / Personal Access Token**, **Base ID**, dan tentukan nama tabel di `.env`.
...
- CRM dashboard otomatis detect mode: jika server aktif → pakai PostgreSQL, jika tidak → fallback ke localStorage
