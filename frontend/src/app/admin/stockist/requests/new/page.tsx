...
import { listProducts, getInventorySummary, createStockRequest, type StockistProduct, type InventoryBalance } from '@/lib/stockistApi';
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const validItems = lines
        ...
        .map((l) => ({ product_id: l.product_id, quantity_requested: Number(l.quantity) }));

      if (validItems.length === 0) {
        throw new Error('Pilih setidaknya satu produk dan jumlahnya');
      }

      const { request } = await createStockRequest({ items: validItems, reason: reason.trim() || undefined });
      router.push(`/admin/stockist/requests/${request.id}`);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-5 animate-fade-in pb-12">
      <div className="flex items-center gap-2">
        ...
        <div>
          <h2 className="text-[20px] font-bold text-text-primary font-display">Ajukan Permintaan Stok</h2>
          <p className="text-[12px] text-text-muted">Minta restock produk dari Gudang Pusat.</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <section className="flex flex-col gap-1.5">
            <label className="text-[10px] font-semibold text-text-muted uppercase tracking-wider">Alasan Permintaan (opsional)</label>
            <textarea
              value={reason}
