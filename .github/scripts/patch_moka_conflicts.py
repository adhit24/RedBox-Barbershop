...
old_queue = '''  if (!existing && !parsedStart && barberId) {
...
new_queue = '''  if (!existing && !parsedStart && barberId) {
...
if old_queue not in s:
    raise SystemExit('Expected goshow queue block not found; refusing unsafe patch')
s = s.replace(old_queue, new_queue, 1)

old_update = '''    if (Object.keys(patch).length > 0) {
      await supabase.from('schedules').update(patch).eq('id', existing.id);
      return 'updated';
    }
'''

new_update = '''    if (Object.keys(patch).length > 0) {
...
if old_update not in s:
    raise SystemExit('Expected update block not found; refusing unsafe patch')
