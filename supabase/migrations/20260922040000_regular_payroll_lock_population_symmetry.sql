-- ====================================================================================================
-- Migration: 20260922040000_regular_payroll_lock_population_symmetry.sql
-- Description:
--   Codex round-12 findings on PR #98 (all against head 4e418185af8faa6c65db6982dc52e565e8ac3dce):
--   1. PRRT_kwDOSNmW7c6klJoV (P1): lock_payroll_run only checked for eligible employees MISSING from
--      the run's item set (the P1-2 backstop, 20260922000000). It never checked the OPPOSITE
--      direction: an existing payroll_regular_items row whose employee is no longer eligible, when the
--      owner locks WITHOUT running recalculateRegularPayrollRun first (so the `population_changed`
--      marker was never even set). Adds an authoritative "extra item" check so the two-way invariant
--      `eligible employee set == payroll_regular_items employee set` holds at lock time regardless of
--      whether recalculation ran first. `population_changed` is kept as defense in depth.
--   2. PRRT_kwDOSNmW7c6klJod (P2): `employees.employment_type = 'regular'` (strict) inside
--      add_regular_payroll_run_items and lock_payroll_run's missing-eligible query did not match the
--      Node-side rule (`employment_type = 'regular' OR IS NULL`, matching the column's own schema
--      DEFAULT). A NULL-employment_type employee -- fully eligible per Node -- could never be added by
--      population reconciliation (the RPC's own eligibility re-check would reject it) and was silently
--      excluded from the "missing eligible" backstop. Both queries, and the new extra-item check, now
--      use the identical `employment_type = 'regular' OR employment_type IS NULL` rule.
--
-- 20260922000000_regular_payroll_population_reconciliation.sql is already applied to production
-- (khcvklzxfohwkyocenaf) and is NOT edited here. This is a new, forward-only corrective migration.
--
...
-- 1. add_regular_payroll_run_items: NULL employment_type accepted, same as the Node-side rule ---------
...
BEGIN
    ...
    FOR v_item IN SELECT * FROM jsonb_populate_recordset(NULL::public.payroll_regular_items, p_items) LOOP
        ...
        -- eligibility snapshot might be. Mirrors the Node-side rule EXACTLY (PRRT_kwDOSNmW7c6klJod):
        -- is_active, employment_type = 'regular' OR NULL (schema DEFAULT is 'regular'), join_date <=
        -- period_end, matching business unit.
        IF NOT EXISTS (
...
-- 2. lock_payroll_run: two-way population equality + NULL employment_type alignment --------------------
